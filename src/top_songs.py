...
def get_top_songs(limit, time_range):
    ...
    for idx, item in enumerate(results['items']):
        ...
        songDict = {'name': name, 'song_id': song_id,
                    'artist_name': artist_name, 'album_image': album_image}
...
def get_top_artists(limit, time_range):
    ...
    for idx, item in enumerate(results['items']):
        ...
        artist = {"artist_name": name,
                  "artist_popularity": popularity, "artist_id": artist_id}
...
def get_featured_playlists(limit):
    results = sp.featured_playlists(
        locale=None, country=None, timestamp=None, limit=limit, offset=0)
...
def recommended_playlist(limit, time_range):
    ...
    new_playlist_id = make_playlist(
        "Simplify Recommendations", True, False, "Your recommended songs! :D")['id']
...
def top_tracks_playlist(limit, time_range):
    title = 'Top songs of the month'
    if (time_range == 'medium_term'):
        title = 'Top songs of the past 6 months'
    elif (time_range == 'long_term'):
        title = 'Top songs of all time'
    
    playlist_id = make_playlist(title, True, False, '')['id']
    playlist_url = sp.playlist(playlist_id)['external_urls']['spotify']

    track_list = get_top_songs(limit, time_range)
    song_ids = []

    for track in track_list:
        song_ids.append(track['song_id'])
